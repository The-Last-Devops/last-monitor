...
function report(h) {
  ...
  return {
    ...
    load5: +(h.cpu / 100 * h.cores * rnd(0.5, 1.0)).toFixed(2),
    load15: +(h.cpu / 100 * h.cores * rnd(0.4, 0.9)).toFixed(2),
    cpu_user: +(h.cpu * rnd(0.5, 0.7)).toFixed(1),
    cpu_system: +(h.cpu * rnd(0.2, 0.35)).toFixed(1),
    cpu_iowait: +rnd(0, 8).toFixed(1),
    cpu_steal: +(h.kind === 'k8s' ? rnd(0, 4) : rnd(0, 1)).toFixed(1),
