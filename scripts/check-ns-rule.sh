#!/usr/bin/env bash
# Verify namespace-wide alert rules: create an "all services in namespace" rule
# (no specific target) and confirm it lists back with scope_kind set. Self-cleaning.
#   bash scripts/check-ns-rule.sh
set -uo pipefail
...
CH=$(curl -s -b "$JAR" -X POST "$BASE/api/namespaces/$NS/channels" -H 'content-type: application/json' \
  -d '{"name":"ns-rule-probe","kind":"webhook","config":{"url":"https://e.com/x"}}' | py "import sys,json;print(json.load(sys.stdin))")

# namespace-wide rule: all services, no monitor_id/system_id
...
say "created namespace-wide rule"; [ -n "$AL" ] && echo "ok ($AL)" || { echo "FAIL"; fail=1; }

say "lists back with scope_kind=all_services"
curl -s -b "$JAR" "$BASE/api/namespaces/$NS/alerts" \
...
BADCODE=$(curl -s -b "$JAR" -o /dev/null -w '%{http_code}' -X POST "$BASE/api/namespaces/$NS/alerts" \
...
[ "$fail" -eq 0 ] && echo "OK" || { echo "ns-rule regressions"; exit 1; }
