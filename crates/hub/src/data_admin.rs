...
pub async fn setup(data: &PgPool) {
    let stmts = [
        // 1-minute rollup from raw metrics.
        "CREATE MATERIALIZED VIEW IF NOT EXISTS metrics_1m \
         WITH (timescaledb.continuous) AS \
         SELECT server_id, time_bucket('1 minute', time) AS bucket, \
         ...
         FROM metrics GROUP BY server_id, bucket WITH NO DATA",
        // 1-hour rollup from raw metrics.
        "CREATE MATERIALIZED VIEW IF NOT EXISTS metrics_1h \
         WITH (timescaledb.continuous) AS \
         SELECT server_id, time_bucket('1 hour', time) AS bucket, \
                avg(cpu_percent) AS cpu_percent, avg(mem_used) AS mem_used, avg(mem_total) AS mem_total, \
                avg(disk_used) AS disk_used, avg(disk_total) AS disk_total, \
                max(net_rx) AS net_rx, max(net_tx) AS net_tx, avg(load1) AS load1 \
         FROM metrics GROUP BY server_id, bucket WITH NO DATA",
        // Refresh policies.
        "SELECT add_continuous_aggregate_policy('metrics_1m', start_offset => INTERVAL '6 hours', \
            end_offset => INTERVAL '1 minute', schedule_interval => INTERVAL '1 minute')",
        "SELECT add_continuous_aggregate_policy('metrics_1h', start_offset => INTERVAL '3 days', \
            end_offset => INTERVAL '1 hour', schedule_interval => INTERVAL '1 hour')",
        // Retention tiers: raw 1 day, 1m 7 days, 1h 30 days.
        "SELECT add_retention_policy('metrics_1m', INTERVAL '7 days')",
    ...
    // Tighten raw retention to 1 day (the default migration set 7 days).
    let _ = sqlx::query("SELECT remove_retention_policy('metrics', if_exists => true)")
        .execute(data)
        .await;
    let _ = sqlx::query("SELECT add_retention_policy('metrics', INTERVAL '1 day')")
        .execute(data)
        .await;
    tracing::info!("downsampling + retention configured");
}
...
pub async fn stats(data: &PgPool) -> DataStats {
    ...
    let tables = vec![
        hypertable_stat(data, "metrics", "Raw metrics").await,
        hypertable_stat(data, "metrics_1m", "1-minute rollup").await,
        hypertable_stat(data, "metrics_1h", "1-hour rollup").await,
        hypertable_stat(data, "container_stats", "Container stats").await,
    ...
    let retention = vec![
        RetentionTier {
            ...
            days: retention_days(data, "metrics").await,
        },
        RetentionTier {
            table: "metrics_1m".into(),
            label: "1-minute rollup".into(),
            days: retention_days(data, "metrics_1m").await,
        },
        RetentionTier {
            table: "metrics_1h".into(),
            label: "1-hour rollup".into(),
            days: retention_days(data, "metrics_1h").await,
        },
        RetentionTier {
            table: "container_stats".into(),
            label: "Container stats".into(),
            days: retention_days(data, "container_stats").await,
        },
        RetentionTier {
