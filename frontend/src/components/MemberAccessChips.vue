...
<template>
  ...
  <div v-else class="flex flex-wrap gap-1.5">
    <span v-for="m in access" :key="m.namespace_id" class="inline-flex items-center gap-1 rounded-md border border-line bg-surface2 px-2 py-0.5 text-xs text-fg">
      {{ nameOf(m.namespace_id) }}<span class="text-faint" :class="{ 'text-accent': m.role === 'owner', 'text-amber-400': m.role === 'editor' }">· {{ nsRoleLabel(m.role) }}</span><span v-if="m.can_exec" v-tip="'Shell access'" class="ml-0.5 rounded bg-accent/15 px-1 text-[10px] font-medium text-accent">shell</span>
    </span>
  </div>
</template>
