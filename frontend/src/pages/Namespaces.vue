...
async function create() {
  ...
  creating.value = true
  try { await api.post('/api/namespaces', { name }); newName.value = ''; await load() }
  catch (e) { err.value = e.status === 500 ? 'A namespace with that name already exists.' : `Failed (${e.status}).` }
...
<template>
  <AppShell title="Namespaces">
    <div class="space-y-5">
      ...
      <template v-else>
        <p v-if="!rows.length" class="rounded-2xl border border-line bg-surface/50 p-10 text-center text-sm text-muted">No namespaces yet.</p>
        <div v-else class="grid gap-3.5 sm:grid-cols-2 xl:grid-cols-3">
          <button v-for="ns in rows" :key="ns.id" @click="open(ns)"
            ...
            <div class="flex items-center gap-2.5">
              ...
              <span class="truncate text-base font-semibold text-fg">{{ ns.name }}</span>
              <span v-if="ns.name === 'default'" class="rounded bg-surface2 px-1.5 py-0.5 text-[10px] uppercase tracking-wider text-faint">default</span>
